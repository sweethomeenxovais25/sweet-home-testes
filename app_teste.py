            ...
            if c_btn2.button("Finalizar Venda 🚀", type="primary", use_container_width=True):
                ...
                with st.spinner("Salvando venda e gerando recibo..."):
                    try:
                        ...
                        nome_primeiro = nome_cli.split(' ')[0]
                        recibo_texto = (
                            f"👩‍💼 *Vendedora:* {vendedor}\n\n"
                            f"✨ *Muito obrigada pela preferência!*\n"
                            f"É um prazer atender você, {nome_primeiro}.\n"
                        ...
                        recibo_texto += f"\n━━━━━━━━━━━━━━━━━━━\n"
                        recibo_texto += f"💰 Subtotal: R$ {subtotal_venda:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") + "\n"
                        
                        if desc_v > 0:
                            recibo_texto += f"📉 Desconto: R$ {desc_v:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") + "\n"
                        ...
                        # Bloco do Sweet Flex (Parcelamento)
                        if metodo == "Sweet Flex":
                            recibo_texto += f"\n📝 Plano de pagamento ({n_p}x):\n"
                            for i, data_p in enumerate(detalhes_p):
                                val_parc = total_com_desconto / n_p
                                recibo_texto += f"🔹 {data_p} — R$ {val_parc:,.2f}\n".replace(",", "X").replace(".", ",").replace("X", ".")
            ...
            if len(dados_v) > 1:
                ...
                if vendas_filtradas:
                    ...
                    if venda_selecionada != "---":
                        ...
                        # 💡 RESGATE DOS DADOS DO SWEET FLEX (Colunas Q e R)
                        try: parc_atual = int(linha_dados[16]) if len(linha_dados) > 16 and str(linha_dados[16]).strip() else 1
                        except: parc_atual = 1
                        
                        venc_atual_str = str(linha_dados[17]) if len(linha_dados) > 17 and str(linha_dados[17]).strip() != "-" else ""
                        ...
                        with st.form(f"form_edicao_{linha_real}"):
                            ...
                            # 💡 NOVOS CAMPOS: Configuração do Sweet Flex
                            st.markdown("---")
                            ...
                            if salvar:
                                try:
                                    ...
                                    val_parc = n_t_liq / num_parc_final if eh_parc == "Sim" else 0
                                    ...
                                    atualizacoes = [
                                        ...
                                        {'range': f'Q{linha_real}', 'values': [[num_parc_final]]},  # 💡 Atualiza as Parcelas
                                        {'range': f'R{linha_real}', 'values': [[venc_final]]},      # 💡 Atualiza o Vencimento
